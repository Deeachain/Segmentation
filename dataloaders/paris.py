...
class ParisDataset(BaseDataSet):
    def __init__(self, mode='fine', **kwargs):
        self.num_classes = 3
        self.mode = mode
        self.palette = palette.Paris_palette
        self.id_to_trainId = ID_TO_TRAINID
        super(ParisDataset, self).__init__(**kwargs)
    ...
    def _set_files(self):
        ...
        self.files = list(zip(image_paths[:2000], label_paths[:2000]))
...
class Paris(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, crop_size=None, base_size=None, scale=True, num_workers=1, mode='fine', val=False,
        ...
        self.dataset = ParisDataset(mode=mode, **kwargs)
        super(Paris, self).__init__(self.dataset, batch_size, shuffle, num_workers, val_split)
